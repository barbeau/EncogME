...
public final class MatrixMath {
	...
	public static double dotProduct(final Matrix a, final Matrix b) {
		...
		for (int i = 0; i < length; i++) {
            ...
            result += aArray[i].doubleValue() * bArray[i].doubleValue();  //sjb - added double values
		}
	...
	public static double vectorLength(final Matrix input) {
		...
		for (int i = 0; i < v.length; i++) {
			//rtn += org.encog.util.Math.pow(v[i], 2);
            rtn += org.encog.util.Math.pow(v[i].doubleValue(), 2); //sjb - added double value
		}
